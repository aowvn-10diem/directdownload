(function() {
    ...
    const script = `
        ...
        const adClickedTime = localStorage.getItem('adClickedTime');
        const now = Date.now();

        if (adClickedTime && now - adClickedTime < 30 * 60 * 1000) return;
        ...
        adImage.onclick = function() {
            ...
            localStorage.setItem('adClickedTime', Date.now());
            setTimeout(() => {
        ...
        adText.textContent = 'Click to explore more!';
